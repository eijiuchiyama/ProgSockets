...
def test_falha():
    ...
    try:
        ...
        print("[Cliente Falho] Simulando fechamento abrupto de conexão (socket.close())...")
        s_falho.close()
        
        time.sleep(0.5)
        
        s_novo = connect_server()
        print(f"[Cliente Novo] Tentando reservar o recurso {res_id} após a falha...")
        resp_novo = send_recv(s_novo, f"RESERVE {res_id}\n")
        print(f"[Cliente Novo] Resposta do RESERVE: {resp_novo}")
        
        print("\n--> Resultado Esperado:")
        print("    Cliente Novo deve receber '👍' (Sucesso), provando que o servidor liberou")
...
if __name__ == '__main__':
    ...
    print(f"Iniciando testes automáticos no servidor {SERVER_IP}:{SERVER_PORT}")
